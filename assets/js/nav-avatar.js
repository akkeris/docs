...
async function getUser(location) {
  let url = location || "/user"
  let headers = new Headers();
  headers.set("accept", "application/json");
  try {
    let res = await fetch(url, {"credentials":"include", "headers":headers})
...
class NavAvatar extends HTMLElement {
  ...
  render() {
    ...
    // attach events
    this.domNode.addEventListener('click', function() { this.domNode.classList.toggle('open') }.bind(this))
    ...
    this.domNode.addEventListener('focusout', function(e) {
  ...
  connectedCallback() {
    (async function() {
      let data = await getUser(this.getAttribute("user-api"));
