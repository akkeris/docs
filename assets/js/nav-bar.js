class NavBar extends HTMLElement {
  ...
  attributeChangedCallback(attr, oldValue, newValue) {
    if(attr === "title") {
      ...
      let button = this.querySelector("button.nav-sidemenu-button")
      if(!button) {
        return
      }
      if(newValue !== "true") {
  ...
  connectedCallback() {
    ...
    this.insertAdjacentHTML('afterbegin',`
      ...
      <h3><span class="title">${this.getAttribute("title") || ""}</span></h3>
    `)

    this.querySelector(".nav-sidemenu-button").addEventListener("click", () => {
      let sidemenus = document.querySelectorAll("nav-sidemenu");
